...
| 选项 | 作用 |
| --- | --- |
| `--include GLOB` | 只处理匹配的文件，可重复给多次 |
| `--exclude GLOB` | 跳过匹配的文件，可重复给多次 |
| `--min-file-size SIZE` | 只处理不小于该体积的文件，如 `1MB` |

通配符同时匹配完整路径和文件名，因此 `--exclude '*/thumbs/*'`（按目录）和 `--exclude '*_draft.jpg'`（按文件名）都能按直觉工作。

```bash
# 只压缩大于 1MB 的 JPEG，跳过缩略图目录
pixshift compress ./photos -r --include '*.jpg' --min-file-size 1MB --exclude '*/thumbs/*'
```

与自动排除派生文件不同，筛选器是你的显式指令，因此对命令行上直接指定的文件同样生效。
...
加 `--backup-dir DIR` 可以把重复文件**移动**到指定目录而不是删除，操作随时可回退：

```bash
pixshift dedup ./photos -r --delete --yes --backup-dir ./_dupes
```

同名文件会自动加数字后缀，不会互相覆盖。确认无误后再删除该目录即可。
